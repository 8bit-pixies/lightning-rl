...
class QNetLightning(pl.LightningModule):
    ...
    def qnet_mse_loss(self, batch):
        ...
        states, _, rewards, _, next_states = batch

        next_state_value = self.net(states)

        with torch.no_grad():
            est_fut_state_value = self.net(next_states).max(1)[0].detach()

        est_fut_state_value * self.gamma + rewards
        return nn.MSELoss()(next_state_value, est_fut_state_value)
